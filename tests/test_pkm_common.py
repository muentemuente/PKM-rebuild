"""Tests für scripts/_pkm_common.py — Drift-Schutz Enums ↔ Taxonomie-Facade.

Der Sinn von _pkm_common ist Single Source of Truth: die Skript-Enums dürfen
nicht von der kanonischen Taxonomie-Facade (pipeline.taxonomy, geladen aus den
config/-YAMLs) abweichen. Diese Tests machen Drift unmöglich. Seit pipeline-v2
ist nicht mehr das Pydantic-`Literal`, sondern `pipeline.taxonomy` die Quelle
(Runtime-Membership-Check); vgl. tests/test_taxonomy.py.
"""
...
def test_allowed_type_matches_taxonomy() -> None:
    assert common.ALLOWED_TYPE == taxonomy.ALLOWED_TYPE


def test_allowed_status_matches_taxonomy() -> None:
    assert common.ALLOWED_STATUS == taxonomy.ALLOWED_STATUS


def test_allowed_review_matches_taxonomy() -> None:
    assert common.ALLOWED_REVIEW == taxonomy.ALLOWED_REVIEW


def test_allowed_confidence_matches_taxonomy() -> None:
    assert common.ALLOWED_CONFIDENCE == taxonomy.ALLOWED_CONFIDENCE


def test_allowed_doc_role_matches_taxonomy() -> None:
    assert common.ALLOWED_DOC_ROLE == taxonomy.ALLOWED_DOC_ROLE


def test_allowed_categories_match_vault_mapping() -> None:
    """ALLOWED_CATEGORIES == kanonische Vault-Ordner-Keys (phase_9 re-export)."""
    assert set(CATEGORY_TO_FOLDER) == common.ALLOWED_CATEGORIES
    assert common.ALLOWED_CATEGORIES == taxonomy.ALLOWED_CATEGORIES
...
def test_required_fields_subset_of_schema() -> None:
    ...
    assert set(FrontmatterDraft.model_fields) >= common.REQUIRED_FIELDS
