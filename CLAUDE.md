...
**Aktueller Stand (2026-07-02):** Basis-Pipeline (Phasen 0–12, **Option B** — Pro-Doc, kein Cross-Doc-Merge) **abgeschlossen** → **181 Vault-Artikel + 5 MOC** (Live-Messung) im Brain-Vault. Embedding-Clustering wurde verworfen (Korpus ohne inhärente Cluster-Struktur). **v3-Zyklus: WP0–WP4 abgeschlossen + gemergt** (WP2 entfiel; Stabilisierung, additive Synthese/MOC, Bestands-Remediation); Vault idempotent. Post-WP4: Q1/Q1b (Quality-Layer), Tier-1-Sweep, H3, **A2a/A2b NB-Backfill promotet** — NB-Felder (`key_points`/`open_questions`/`next_steps`/`keyphrases`) auf allen 165 produktiv+nutzbaren Notes (NB-Linie geschlossen). Menschlich verbleibend: Qualitätsstufe-2-Review. Plan: `docs/Projektplan_pipeline-v3.md`, aktueller Detailstand: `docs/PROJECT_STATUS.md`; Bedienung: `docs/MANUAL.md`.
...
- 2026-07-02 — **NB-Linie geschlossen** + Promote-Fix (PR #57): `_DRAFT_OVERLAY_FIELDS` (`pipeline/promotion.py`) verwarf die additive NB-/Keyphrase-Ebene beim Update-Promote still → der A2a/A2b-Backfill landete nicht im Vault (165 OK im Log, 0 Felder im Vault). Fix (`keyphrases`/`key_points`/`open_questions`/`next_steps` in die Overlay-Whitelist) + Regressionstest; Re-Promote der 165 Drafts aus `archive/promoted_drafts/` (165/165 OK). NB-Felder auf allen 165 Notes verifiziert. §1 Stand-Block auf 2026-07-02 (NB-Linie, MANUAL-Pointer). Doc-Sync-Runde: README neu gefasst, neues `docs/MANUAL.md`, Spec §4 + Vault-Standard §3 (NB-Ebene), PROJECT_STATUS/FUTURE_RUN nachgeführt. Test-Suite 843 passed / 2 skipped
