...
**Basis-Pipeline (0–12, Option B — Pro-Doc, kein Cross-Doc-Merge) abgeschlossen. v3-Zyklus WP0–WP4 gemergt. Q1/Q1b + Tier-1-Sweep + H3 gemergt. A2a/A2b NB-Backfill: Drafts human-reviewed und **promotet** — die NB-Felder (`key_points`/`open_questions`/`next_steps`/`keyphrases`) stehen auf allen 165 produktiv+nutzbaren Notes (Promote-Fix PR #57). NB-Linie geschlossen. Vault produktiv und idempotent. Menschlich verbleibend: Qualitätsstufe-2-Review.**
...
| Test-Suite | **806** grün (`def test_`; 843 passed / 2 skipped parametrisiert), `ruff` clean, `mypy pipeline/` clean |
...
| **A2a** NB-Backfill (Hub) | additiver Feld-Backfill (`key_points`/`open_questions`/`next_steps`) für 9 Hub-Kandidaten; Code + 9 Drafts | ✅ Code gemergt (PR #53); Drafts human-reviewed + **promotet** |
| **A2b** NB-Backfill (Band) | selber Mechanismus über **156 Files** (produktiv+nutzbar − 9 A2a-Hub); 0 nacharbeit-Band in diesem Score-Run | ✅ **156/156 gedraftet, 0 Fehler** (Live-Qwen, Ø 108,6 s/File) + **promotet** |
| **Promote-Fix** (PR #57) | `_DRAFT_OVERLAY_FIELDS` (`promotion.py`) verwarf die additive NB-/Keyphrase-Ebene beim Update-Promote still → NB-Felder landeten nicht im Vault. Fix + Regressionstest, Re-Promote aus `archive/promoted_drafts/` (165/165 OK) | ✅ gemergt (PR #57), Re-Promote ausgeführt |
...
**A2-Eigenschaften (verifiziert):** Body byte-identisch (nur 3 FM-Felder additiv ergänzt); Anti-Halluzination greift (`open_questions: []` wenn nichts Grounded); `verify_additive` per File. Review-Sheet: `docs/handover/a2b-nb-backfill-review.md`. **Promote (2026-07-02):** 165/165 Notes aktualisiert (`status` bleibt `review`, nie auto-`stable`), pro File Rollback-Snapshot; NB-Felder im Vault verifiziert (165× `key_points`/`open_questions`/`next_steps`/`keyphrases`).
...
| Tool | Status |
|---|---|
| `pytest` | ✅ **806** grün (843 passed / 2 skipped parametrisiert) |
...
| Punkt | Status |
|---|---|
| **A2a/A2b NB-Backfill** (165 Notes) | ✅ **erledigt** — human-reviewed + promotet (2026-07-02); NB-Felder im Vault verifiziert |
...
- **Qualitätsstufe-2-Review** der Bestandsartikel (`draft → review/stable`) — manuell, kein CC-Auto-Promote. Das ist der unmittelbar nächste menschliche Schritt (A2-Backfill ist abgeschlossen).
...
- 2026-07-02 — **NB-Linie geschlossen:** Promote-Fix (PR #57, `_DRAFT_OVERLAY_FIELDS` verwarf NB-/Keyphrase-Ebene beim Update-Promote still) + Re-Promote der 165 A2a/A2b-Drafts aus `archive/promoted_drafts/` (165/165 OK). NB-Felder (`key_points`/`open_questions`/`next_steps`/`keyphrases`) auf allen 165 Notes verifiziert. §0/§2b/§7/§8/§9 nachgeführt (A2 promotet, 806/843 Tests, A2-Sichtung erledigt, nächster Schritt = Q-Stufe-2). Doc-Sync-Runde (README/MANUAL/Spec/Vault-Standard)
